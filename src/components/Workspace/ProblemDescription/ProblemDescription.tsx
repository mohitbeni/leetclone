...
const ProblemDescription: React.FC<ProblemDescriptionProps> = ({
  ...
  const handleLike = async () => {
    ...
    await runTransaction(firestore, async (transaction) => {
      const { userDoc, userRef, problemDoc, problemRef } =
        await returnUserDataAndProblemData(transaction)
      if (userDoc.exists() && problemDoc.exists()) {
        if (liked) {
          //remove problem id from likedProblems array and decrement likes
  ...
  const handleDislike = async () => {
    ...
    //if already disLiked, if already liked, neither
    if (updating) return
    setUpdating(true)
    await runTransaction(firestore, async (transaction) => {
      const { userDoc, userRef, problemDoc, problemRef } =
      ...
      if (userDoc.exists() && problemDoc.exists()) {
        if (disliked) {
          //remove problem id from dislikedProblems array and decrement likes
          transaction.update(userRef, {
            dislikedProblems: userDoc
              .data()
              .dislikedProblems.filter((id: string) => id !== problem.id),
  ...
  useEffect(() => {
    ...
    const getCurrentProblem = async () => {
      ...
      if (docSnap.exists()) {
        ...
        // console.log('current problem is: ', problem)
